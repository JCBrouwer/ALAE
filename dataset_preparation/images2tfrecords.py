import zipfile
import tqdm
from defaults import get_cfg_defaults
import sys
import logging

import dlutils
from dlutils import download

from scipy import misc
from net import *
import numpy as np
import pickle
import random
import argparse
import os

# HACK cuz otherwise `AttributeError: module 'dlutils' has no attribute 'use_cuda'`
dlutils.use_cuda = True
from dlutils.pytorch.cuda_helper import *
import tensorflow as tf
import imageio
from PIL import Image


def prepare_dataset(cfg, logger):
    directory = os.path.dirname(cfg.DATASET.PATH)

    os.makedirs(directory, exist_ok=True)

    images = []
    source_path = "/home/hans/datasets/alaeset/"  # cfg.DATASET.SOURCE_PATH
    for filename in tqdm.tqdm(os.listdir(source_path)):
        images.append(filename)

    print("Total count: %d" % len(images))
    images = images[: cfg.DATASET.SIZE]

    count = len(images)
    print("Count: %d" % count)

    random.seed(0)
    random.shuffle(images)

    folds = cfg.DATASET.PART_COUNT
    dataset_folds = [[] for _ in range(folds)]

    count_per_fold = count // folds
    for i in range(folds):
        dataset_folds[i] += images[i * count_per_fold : (i + 1) * count_per_fold]

    for i in range(4, folds):
        path = cfg.DATASET.PATH

        writers = {}
        for lod in range(cfg.DATASET.MAX_RESOLUTION_LEVEL, 1, -1):
            tfr_opt = tf.python_io.TFRecordOptions(tf.python_io.TFRecordCompressionType.NONE)
            part_path = path % (lod, i)
            os.makedirs(os.path.dirname(part_path), exist_ok=True)
            tfr_writer = tf.python_io.TFRecordWriter(part_path, tfr_opt)
            writers[lod] = tfr_writer

        for filename in tqdm.tqdm(dataset_folds[i]):
            try:
                img = np.asarray(Image.open(os.path.join(source_path, filename)))
                if len(img.shape) != 3:
                    img = np.stack([img, img, img], axis=2)
                img = img.transpose((2, 0, 1))
            except:
                while True:
                    print(filename, "failed...")
                    try:
                        all_imgs = os.listdir("/home/hans/datasets/animacht/imgs/")
                        filename = np.random.choice(all_imgs)
                        print(f"/home/hans/datasets/animacht/imgs/{filename}")
                        img = np.asarray(Image.open(f"/home/hans/datasets/animacht/imgs/{filename}"))
                        if len(img.shape) != 3:
                            img = np.stack([img, img, img], axis=2)
                        img = img.transpose((2, 0, 1))
                        print(img.shape)
                        break
                    except:
                        print(filename, "failed again...")

            for lod in range(cfg.DATASET.MAX_RESOLUTION_LEVEL, 1, -1):
                ex = tf.train.Example(
                    features=tf.train.Features(
                        feature={
                            "shape": tf.train.Feature(int64_list=tf.train.Int64List(value=img.shape)),
                            "data": tf.train.Feature(bytes_list=tf.train.BytesList(value=[img.tostring()])),
                        }
                    )
                )
                writers[lod].write(ex.SerializeToString())

                image = torch.tensor(np.asarray(img, dtype=np.float32)).view(1, 3, img.shape[1], img.shape[2])
                image_down = (
                    F.avg_pool2d(image, 2, 2)
                    .clamp_(0, 255)
                    .to("cpu", torch.uint8)
                    .view(3, image.shape[2] // 2, image.shape[3] // 2)
                    .numpy()
                )

                img = image_down


def run():
    parser = argparse.ArgumentParser(description="Adversarial, hierarchical style VAE")
    parser.add_argument(
        "--config-file", default="configs/flood-param.yaml", metavar="FILE", help="path to config file", type=str,
    )
    parser.add_argument(
        "opts", help="Modify config options using the command-line", default=None, nargs=argparse.REMAINDER,
    )

    args = parser.parse_args()
    cfg = get_cfg_defaults()
    cfg.merge_from_file(args.config_file)
    cfg.merge_from_list(args.opts)
    cfg.freeze()

    logger = logging.getLogger("logger")
    logger.setLevel(logging.DEBUG)

    output_dir = cfg.OUTPUT_DIR
    os.makedirs(output_dir, exist_ok=True)

    ch = logging.StreamHandler(stream=sys.stdout)
    ch.setLevel(logging.DEBUG)
    formatter = logging.Formatter("%(asctime)s %(name)s %(levelname)s: %(message)s")
    ch.setFormatter(formatter)
    logger.addHandler(ch)
    logger.info(args)

    logger.info("Loaded configuration file {}".format(args.config_file))
    with open(args.config_file, "r") as cf:
        config_str = "\n" + cf.read()
        logger.info(config_str)
    logger.info("Running with config:\n{}".format(cfg))

    prepare_dataset(cfg, logger)


if __name__ == "__main__":
    run()

